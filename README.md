...
按下 **⌥A** 后拖动选择区域；按 Esc 静默取消。成功的 PNG 会进入系统剪贴板并作为普通图片加入历史，不会弹出预览或保存到桌面。面板内可搜索文本、切换「纯文本」粘贴（仅影响文本）。在「偏好设置」中可以录制、恢复或关闭截屏快捷键，也可分别调整最多保留的条数和保留时长；默认仍为 **200 条 / 7 天**。将时长设为「永久」会停用按时间清理，但条数上限仍会继续生效。
